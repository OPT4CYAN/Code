...
#tipos de plots que pueden interesarnos (no uso todos)
...
def ndci_plot(xeje,ndci_fil,sentinel_ndci,output_dir,save_name,filtro)->float:
    fig                     =plt.figure(figsize=(20,15))
    ax                      =fig.add_axes([0.1,0.1,0.8,0.8])
    ax.plot(xeje,ndci_fil,color="blue",label="ndci_TriOS")
...
def chl_pcu(xeje,yeje,std,pcu,std_2,output_dir,save_name,filtro)->float:
    fig                     =plt.figure(figsize=(20,15))
    ax                      =fig.add_axes([0.1,0.1,0.8,0.8])
    ax.plot(xeje,yeje,color="green",label="Chlorophyll index")
    ax.plot(xeje,pcu,color="blue",label="Cyanobacteria index")
    ax.fill_between(xeje, yeje - std, yeje + std, color="#013d02", alpha=0.2, label="Std Chlorophyll")
    ax.fill_between(xeje, pcu  - std_2, pcu + std_2, color="#00BFFF", alpha=0.2, label="Std Cyanobacteria")
    if len(xeje)>=20:
        ax.xaxis.set_major_locator(MaxNLocator(nbins=20, integer=True))
    for tick in ax.get_xticklabels():
        tick.set_rotation(45)
