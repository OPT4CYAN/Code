...
#def que carga y produce los plots
def plot_run (dir_input,today,site,sentinel_dir,web_dir)->str:
    
    web_site= web_dir+site+"/"
...
###abrimos todos los archivos producidos tanto filtrados como no
    ...
    chl_pcu(normal_fech,dirs_data['chl_index_fil'],dirs_data['chlstd_index_fil'],dirs_data['pcu_index_fil'],dirs_data['pcustd_index_fil'],web_site,"chlpcu_alldays",False)
    
    ndci_plot(normal_fech,dirs_data['ndci_index_fil'],dirs_data[site],web_site,"ndci_alldays",False)
    ...
    if len(ID_int)>7:
        ...
        if days_7:
            ...
            chl_pcu(days7,chl_7_fil,chlstd_7_fil,pcu_7_fil,pcustd_7_fil,web_site,"chlpcu_7d",None)
            chl_pcu(days7,chl_7_fil,chlstd_7_fil,pcu_7_fil,pcustd_7_fil,output_dir+"chl/",str(normal_today_str)+"_7d",None)
            
            ndci_plot(days7,ndci_7_fil,sen_7,web_site,"ndci_7d",None)
            ndci_plot(days7,ndci_7_fil,sen_7,output_dir+"ndci/",str(normal_today_str)+"_7d",None)
    ...
    if len(ID_int)>30:
        ...
        if days_30:
            ...
            chl_pcu(days30,chl_30_fil,chlstd_30_fil,pcu_30_fil,pcustd_30_fil,web_site,"chlpcu_30d",None)
            chl_pcu(days30,chl_30_fil,chlstd_30_fil,pcu_30_fil,pcustd_30_fil,output_dir+"chl/",str(normal_today_str)+"_30d",None)
    ...
    for y in uniq_year:
        ...
        rrs_plot(wl,year_rrs,web_site,"rrs_year")
        ndci_plot(year_fech,year_ndci_dif,year_ndci_sen,output_dir+"ndci/",str(y),False)
        ndci_plot(year_fech,year_ndci_dif,year_ndci_sen,web_site,"ndci_year",False)
        chl_pcu(year_fech,year_chl_dif,year_chl_std,year_pcu_dif,year_pcu_std,output_dir+"chl/",str(y),False)
        chl_pcu(year_fech,year_chl_dif,year_chl_std,year_pcu_dif,year_pcu_std,web_site,"chlpcu_year",False)
        ...
        for i in range(len(month)):
            if len(days[month_name[i]])!=0:
                ...
                chl_pcu(days[month_name[i]],month_chl_dif[month_name[i]],month_chl_std[month_name[i]],month_pcu_dif[month_name[i]],month_pcu_std[month_name[i]],web_site,"chlpcu_"+month_name[i]+"_year",True)
                ...
                chl_pcu(days[month_name[i]],month_chl_dif[month_name[i]],month_chl_std[month_name[i]],month_pcu_dif[month_name[i]],month_pcu_std[month_name[i]],web_site,"ndci_"+month_name[i]+"_year",True)
                ndci_plot(days[month_name[i]],month_ndci_dif[month_name[i]],month_ndci_sen[month_name[i]],output_dir+"ndci/",month_name[i],True)
    
    #aleeta system, today-3 >20% red alert, today-1 >50% red alert, 
    alerta =False
    if len(ID_int)>3:
        normal_today = datetime.strptime(str(today)[0:4] + str(today)[4:], "%Y%j")
        normal_today_str = normal_today.strftime("%d-%m-%Y")
        day_3 = np.array([(normal_today - timedelta(days=i)).strftime("%d-%m-%Y") for i in range(3)])
        days_3 = [day for day in day_3 if day in normal_fech]
        if days_3:
            indice = [np.where(normal_fech == day)[0][0] for day in days_3]
            # Determinar el rango de días válidos
            days3 = normal_fech[min(indice):max(indice) + 1]
            pcu_3_fil=dirs_data['pcu_index_fil'][indice]
            
            
        else:
            days3 = np.array([]) 
    if len(days3)==3:
        porcent=abs(np.nanmax(pcu_3_fil)-np.nanmin(pcu_3_fil))/((np.nanmax(pcu_3_fil)+np.nanmin(pcu_3_fil))/2)*100
        if porcent >20:
            alerta=None
    if len(days3)==3:
        porcent=abs(np.nanmax(pcu_3_fil)-np.nanmin(pcu_3_fil))/((np.nanmax(pcu_3_fil)+np.nanmin(pcu_3_fil))/2)*100
